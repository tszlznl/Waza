...
1. 先去 AI 味：删模板句、删讲解腔、删报告腔。**首先检查段末收尾总结句**（"这说明"、"到这里"、"可以看出"开头的重述句），这是最高频的 AI 痕迹，优先删。
...
- 句子长度都差不多，是模型腔里最好量的一条。人写的段落里十个字的句子会挨着四十个字的句子；一段读下来长度接近，就挑一句压到十字以内，或者放开一句让它长过三十字。段落长度同理，连续几段都差不多长，检查是不是每段都按同一副模具做出来的
- 连词删掉一半再读。"因为 / 所以 / 但是 / 同时 / 此外 / 然而"每两句就来一个是模型习惯，中文小句靠语序和事理就能相接，读不断的地方不用补回来。真在转折、接时间或带口气的连词保留，只删按固定位置出现、拿掉也不影响意思的那些
...
> OK: 直接给出具体建议，不升华
> 结尾自查两步：把最后两段分别删掉再读，删掉更有力就让文章提前结束；搜"时代、文明、未来、世界、历史、所有人"，正文没有一直在处理这个尺度，结尾就回到具体事实和当前判断。开头的物件在结尾回来，要确认读者对它的理解已经变了，含义没变就不必回来打卡。
...
> 文章确实用材料走过了从误解到修正的过程时，可以保留一次自我修正，但不能穿上面任何一件外衣。同一副骨架在一篇里出现 2 次以上，留最有材料的一处，其余全部拆成正面陈述。
